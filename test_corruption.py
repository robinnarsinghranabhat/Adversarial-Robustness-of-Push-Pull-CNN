...
parser.add_argument('--fgsm-epsilon', default=0.0, type=float, help='Epsilon for FGSM attack (default: 0.0)') 
...
def main():
    ...
    prec1 = validate(clean_loader, model, criterion, adversarial_eps=fgsm_epsilon, file=fileout)
    ...
    for distortion_name in distortions:
        rates, accuracies = validate_corrupted(distortion_name, model, criterion, adversarial_eps=fgsm_epsilon)
    ...
    f1.close()
    f2.close()
    print('## -- Overall Mean Error on all distorted CIFAR images at each severity : {:.2f}'.format(100 * np.mean(mean_rates)))
    print('## -- Overall Mean Accuracty on all distorted CIFAR images at each severity: {:.2f}'.format(np.mean(mean_acc_rates)))


def validate_corrupted(distortion_name, model, criterion=None, adversarial_eps=0):
...
def validate(val_loader, model, criterion, adversarial_eps=0, file=None):
    ...
    for i, (input, target) in enumerate(val_loader):
        ...
        if adversarial_eps == 0:
            ...
            output = model(input_var)
            loss = criterion(output, target_var)
            # Make sure no accumulated gradients  
            model.zero_grad()
            loss.backward()
